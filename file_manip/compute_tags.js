function computeTags(text, types) {
  ...
  */
  ...
  if (/when you gain/i.test(selfText) && !/when you gain a card/i.test(selfText)) tags.add('on_gain');
  ...
  if (/top of their deck|top of (?:each )?other player|-1 card token/i.test(oppSections)) tags.add('deck_attack');
  ...
  if (
    ...
    (
      ...
      || /choose one.*or trash/i.test(selfText)
  ...
  if (/trash\b.*gain|trash.*to gain/i.test(selfText) && !/trash.*or.*gain/i.test(selfText)) tags.add('trash_to_gain');
  ...
  // DECK CONTROL (self)
  if (/look.*the top|reveal.*top|top \d+ cards of your deck|reveal.*until/i.test(selfText) && !/look.*top card.*hand/i.test(selfText)) tags.add('scry');
