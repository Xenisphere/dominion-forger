function computeTags(text, types) {
  ...
  */
  ...
  // TRASHING (self)
  if (((tags.has('trash') /*&& !/or.*trash/i.test(selfText) && !/trash.*;.*or/i.test(selfText)*/) || /trash.*choose/i.test(selfText)) && (tags.has('+cards') || tags.has('+card') || tags.has('+coins') || tags.has('+action') || tags.has('+actions') || tags.has('gain') || /trash.*into your hand/i.test(selfText))) tags.add('trash_for_benefit');
