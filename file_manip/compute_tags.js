function computeTags(text, types) {
  ...
  // TRASHING (self)
  if (tags.has('trash') && (tags.has('+cards') || tags.has('+card') || tags.has('+coins') || tags.has('+action') || tags.has('+actions') || tags.has('gain'))) tags.add('trash_for_benefit');
  ...
  if (isAction && !hasAnyAction && !/play.*action.*twice|play.*twice/i.test(selfText)) tags.add('terminal');
  ...
  if (tags.has('terminal') && tags.has('draw')) tags.delete('terminal');
